...
const copyPersonalInfo = () => {
    setFieldValue('businessName', fullName.value)
    setFieldValue('businessEmail', email.value)
}
...
const handleSignup = handleSubmit(async (values) => {
  ...
  try {
    ...
    displayError(error, 'An unexpected error occurred during registration')
  ...
  <div class="relative min-h-screen flex items-center justify-center bg-gray-50 dark:bg-[#02040a] px-4 py-10 overflow-hidden text-gray-900 dark:text-white font-sans selection:bg-indigo-500/30 transition-colors duration-300">
    ...
    <div class="w-full max-w-lg relative z-10 animate-fade-in-up">
      <div class="text-center mb-8 space-y-2">
        <router-link to="/" class="text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white transition-colors text-sm font-medium mb-4 inline-block">
          ← Back to Home
        </router-link>
        <h1 class="text-3xl font-bold tracking-tight">Create your account</h1>
        <p class="text-gray-500 dark:text-slate-400">Join the thousands of people creating experiences</p>
      </div>
      ...
      <div v-if="isPendingApproval" class="rounded-3xl border border-indigo-500/20 bg-indigo-500/5 backdrop-blur-xl p-8 text-center shadow-2xl">
        ...
        <div class="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-1.5 mb-8 overflow-hidden">
          <div class="h-full bg-indigo-500 rounded-full animate-progress" style="width: 100%"></div>
        </div>
        <button type="button" class="w-full py-3.5 rounded-full bg-gray-900 dark:bg-white text-white dark:text-black font-semibold hover:bg-gray-700 dark:hover:bg-slate-200 transition-colors" @click="router.push('/login')">
          Go to Login
        </button>
      </div>

      <div v-else class="rounded-3xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/[0.02] backdrop-blur-xl p-6 sm:p-8 shadow-2xl transition-colors duration-300">
        <form class="space-y-6" @submit.prevent="handleSignup">
          ...
          <!-- User Type Selection -->
          <div class="p-1 rounded-2xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5 flex gap-1">
            <button type="button"
              class="flex-1 py-2.5 rounded-xl font-medium text-sm transition-all duration-300"
              :class="userType === 'attendee' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-white/5'"
              @click="userType = 'attendee'" :disabled="authStore.loading">
              Attendee
            </button>
            <button type="button"
              class="flex-1 py-2.5 rounded-xl font-medium text-sm transition-all duration-300"
              :class="userType === 'organizer' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-white/5'"
          ...
          <div class="space-y-5">
            <div class="space-y-2">
              <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 ml-1">Full Name</label>
              ...
              <p v-if="errors.fullName" class="text-xs text-red-500 dark:text-red-400 ml-1">{{ errors.fullName }}</p>
            </div>
            ...
            <div class="space-y-2">
              <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 ml-1">Phone Number</label>
              ...
              <p v-if="errors.phoneNumber" class="text-xs text-red-500 dark:text-red-400 ml-1">{{ errors.phoneNumber }}</p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 ml-1">Password</label>
              ...
              <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 ml-1">Confirm Password</label>
                ...
                <p v-if="errors.confirmPassword" class="text-xs text-red-500 dark:text-red-400 ml-1">{{ errors.confirmPassword }}</p>
              </div>
            </div>
          </div>

          <!-- Organizer-specific Fields -->
          <div v-if="userType === 'organizer'" class="pt-6 border-t border-gray-200 dark:border-white/10 animate-fade-in-up">
            <h3 class="text-lg font-semibold mb-6 flex items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                  <span class="text-indigo-600 dark:text-indigo-400">🏢</span> Business Information
              </div>
              <button 
                type="button" 
                @click="copyPersonalInfo"
                class="text-xs bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 px-3 py-1.5 rounded-full hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-colors"
                >
                Use Personal Info
                </button>
            </h3>
            ...
            <div class="space-y-5">
              <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 ml-1">Business Name</label>
                ...
                <p v-if="errors.businessName" class="text-xs text-red-500 dark:text-red-400 ml-1">{{ errors.businessName }}</p>
              </div>

              <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 ml-1">Business Email</label>
                ...
                <p v-if="errors.businessEmail" class="text-xs text-red-500 dark:text-red-400 ml-1">{{ errors.businessEmail }}</p>
              </div>
              
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <!-- Helper Component for File Input -->
                <div class="space-y-2">
                  <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 ml-1">KYC (Front)</label>
                  <div class="relative group">
                    <input type="file" accept="image/*" 
                      class="block w-full text-sm text-gray-500 dark:text-slate-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 dark:file:bg-indigo-500/10 file:text-indigo-600 dark:file:text-indigo-400 hover:file:bg-indigo-100 dark:hover:file:bg-indigo-500/20 cursor-pointer"
                ...
                <div class="space-y-2">
                  <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 ml-1">KYC (Back)</label>
                  <div class="relative group">
                     <input type="file" accept="image/*" 
                      class="block w-full text-sm text-gray-500 dark:text-slate-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 dark:file:bg-indigo-500/10 file:text-indigo-600 dark:file:text-indigo-400 hover:file:bg-indigo-100 dark:hover:file:bg-indigo-500/20 cursor-pointer"
                      :disabled="authStore.loading"
              ...
              <p class="text-xs text-gray-500 dark:text-slate-500 italic">* Upload clear images of ID or Passport</p>
          ...
          <label class="flex items-start gap-3 cursor-pointer group pt-2">
            <div class="relative flex items-center pt-0.5">
              <input v-model="agreeTerms" type="checkbox"
                class="peer h-4 w-4 rounded border-gray-300 dark:border-white/20 bg-gray-50 dark:bg-white/5 text-indigo-600 focus:ring-offset-0 focus:ring-indigo-500/50 cursor-pointer"
                :disabled="authStore.loading" required>
            </div>
            <span class="text-sm text-gray-500 dark:text-slate-400 group-hover:text-gray-700 dark:group-hover:text-slate-300 transition-colors">
          ...
          <p class="text-center text-sm text-gray-500 dark:text-slate-400">
            ...
            <router-link to="/login" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 font-medium hover:underline transition-colors">
              Sign in
            </router-link>
          </p>
        </form>
